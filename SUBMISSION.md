...
- 225 automated tests cover the physics, datasets, scoring, scenarios and exports; `validate:data` schema-checks every JSON dataset in CI.
...
Every change is gated by CI (`.github/workflows/ci.yml`): lint â†’ typecheck â†’ dataset validation â†’ 225 tests â†’ production build. To go live on Vercel:
